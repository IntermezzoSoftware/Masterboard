...
func (e *Chromium) Embed(hwnd uintptr) bool {
	...
	// Masterboard patch: allow the perf harness to inject extra Chromium
	// switches (notably --remote-debugging-port) at Embed time via a
	// process env var. Reading here — inside Embed — runs *after* Wails'
	// init-time preventEnvAndRegistryOverrides(), so the value survives.
	// The variable name is deliberately Masterboard-specific to sidestep
	// Wails' own WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS clobber. See
	// third_party/go-webview2/PATCH_NOTES.md in the Masterboard repo.
